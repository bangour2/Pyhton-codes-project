...
cmd = "select ident, name, president_id from club"
curs.execute(cmd)
...
for cl in allClubs:
    ...
    if len(ismem) > 0:
        ...
        if cl[2] == ident:  # is president
            info.append(1)
            presidents.append(cl[1])
        else:
            info.append(0)  # is not president
    else:
        info.append(0)   # is not a member
        info.append(0)   # is not president either
